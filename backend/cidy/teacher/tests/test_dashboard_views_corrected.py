...
class DashboardViewTestCaseNew(APITestCase):
    def setUp(self):
        ...
        # Note: No need to explicitly set last_status_date as it uses auto_now=True
        # We'll need to manually update it for testing specific dates
        self.class1 = Class.objects.create(
            group_enrollment=self.enrollment1,
            status='attended_and_paid',
            attendance_date=date(2023, 1, 20)
        )
        # Update last_status_date for testing
        Class.objects.filter(pk=self.class1.pk).update(last_status_date=datetime(2023, 1, 20, 12, 0, 0))
        ...
        # Update last_status_date for testing
        Class.objects.filter(pk=self.class2.pk).update(last_status_date=datetime(2023, 2, 5, 12, 0, 0))
        ...
        # Update last_status_date for testing
        Class.objects.filter(pk=self.class3.pk).update(last_status_date=datetime(2023, 3, 15, 12, 0, 0))
        ...
        # Update last_status_date for testing
        Class.objects.filter(pk=self.class4.pk).update(last_status_date=datetime(2023, 2, 25, 12, 0, 0))
        ...
        # Update last_status_date for testing
        Class.objects.filter(pk=self.class5.pk).update(last_status_date=datetime(2023, 3, 10, 12, 0, 0))
    ...
    def test_custom_date_range(self):
        ...
        response = self.client.get(
            f"{reverse('teacher_get_dashboard_data')}?start_date={start_date}&end_date={end_date}"
        )
